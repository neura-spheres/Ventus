...
use crate::{
    ...
    Microsoft::Web::WebView2::Win32::{
        ...
        ICoreWebView2EnvironmentOptions3_Impl, ICoreWebView2EnvironmentOptions6,
        ICoreWebView2EnvironmentOptions6_Impl, ICoreWebView2EnvironmentOptions_Impl,
...
#[allow(clippy::not_unsafe_ptr_arg_deref)]
impl ICoreWebView2EnvironmentOptions6_Impl for CoreWebView2EnvironmentOptions {
    fn AreBrowserExtensionsEnabled(&self, result: *mut BOOL) -> Result<()> {
        if result.is_null() {
            E_POINTER.ok()
        } else {
            unsafe { *result = self.are_browser_extensions_enabled().into() };
            Ok(())
        }
    }
